...
class upravaApp
{
    ...
    public function preulozTabulkuRezervacie()
    {
        ...
        $data = Db::queryAllRows($dopyt, array());
...
//cele to robit vkladanim postupne do DB
            if($da['typ'] === 'gym-trener')// ak ukladam skupinovy tréning
            {
                ...
                if ($da[RezervaciaManazer::OSOBA_ID] === $da['trener_id']) // ulozenie treningu kktory vytvoril trener a teda jedo treningu
                {
                    ...
                    Db::insert('rezervacia', $rezervaciaTab);
                    //echo ("Trener:");
                    //print_r($rezervaciaTab);
                    $rezervaciaId =Db::returnLastId(); //$kluc;
                    ...
                    if(!empty($da['poznamka'])) // ak je poznamkja tak ju ulozim
                    {
                        ...
                        Db::insert('poznamka', $poznamkaTab);
                        //echo ("poznamka:");
                        //print_r($poznamkaTab);
                    }
                }
    ...
    public function vytvorTabulkuRezervacia()
    {
        try {
            // odstránenie Tabulky
            Db::query('DROP TABLE rezervacia ');

            //vytvorenie tabulky
            Db::query('CREATE TABLE IF NOT EXISTS rezervacia (
                                                            rezervacia_id int(11) NOT NULL,
            ...
            //indexy
            Db::query('ALTER TABLE rezervacia
    ...
    public function vytvorTabulkuTrener()
    {
        try {
            //vytvorenie tabulky
            Db::query('CREATE TABLE IF NOT EXISTS trener (
    ...
    {
        try {
            Db::query('ALTER TABLE uzivatel DROP COLUMN trener');
    ...
    public function upravTabulkuKlientov()
    {
        ...
        $klienti = Db::queryAllRows('SELECT * FROM klient ORDER BY klient_id');

        // nove dáta
        if(isset($klienti[0]['osoba_trener_id']))
        {
            $klienti = Pole::filterKeys($klienti, array('osoba_trener_id', 'osoba_klient_id'));
            ...
            try {
                // odstránenie Tabulky
                Db::query('DROP TABLE klient');

                //vytvorenie tabulky
                Db::query('CREATE TABLE IF NOT EXISTS klient (
                                                            klient_id int(11) NOT NULL,
                                                            trener_id int(11) NOT NULL,
                                                            osoba_klient_id int(11) NOT NULL
                                                            ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_slovak_ci');
                //indexy
                Db::query('ALTER TABLE klient
